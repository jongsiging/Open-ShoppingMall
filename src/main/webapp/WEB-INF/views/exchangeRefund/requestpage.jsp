			...
			<section class="sec-reason-detail">
				...
				<c:if test="${managerLogin == null }">
					<form action="/exchangeRefund/insertExRefund" method="POST">
						...
						<input id="exchange" type="radio" name="exchange_refund_code"
							value="3"><label for="exchange">교환</label> <input
							id="refund" type="radio" name="exchange_refund_code" value="4"><label
							for="refund">환불</label>
						<textarea name="exchange_refund_content" class="form-control"
							rows="5" placeholder="교환/환불사유를 입력해주세요." maxlength="100"></textarea>
						<input type="submit" class="btn btn-primary btn-resist-refund"
							data-dismiss="modal" value="신청">
					</form>
				</c:if>
				<c:if test="${managerLogin != null }">
		...
		$(document)
				.ready(
						function() {
							...
							$.getJSON("/item/getItem_size/" + item_name,
									function(data) {
										for (var i = 0; i < data.length; i++) {
											var msg = "<option>" + data[i]
													+ "</option>";
											$("#item_size").append(msg);
										}
									});

							$.getJSON("/item/getItem_color/" + item_name,
									function(data) {
										for (var i = 0; i < data.length; i++) {
											var msg = "<option>" + data[i]
													+ "</option>";
							...
							$("[type='submit']")
									.click(
											function(event) {
												...
												var textval = $(
														"[name='exchange_refund_content']")
														.val();
												var code = $(
														"input[name='exchange_refund_code']:checked")
														.val();
												if (textval == '') {
													$(
															"[name='exchange_refund_content']")
